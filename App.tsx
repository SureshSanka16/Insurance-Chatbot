...
const LoginScreen = ({ onLogin }: { onLogin: (role: UserRole) => void }) => {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Success - use the user's actual role from backend, not selectedRole
      onLogin(user.role);
    } catch (err: any) {
      // Login failed
      setError(
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Success - use the user's actual role from backend
      onLogin(user.role);
    } catch (err: any) {
    ...
    return (
      <div className="h-screen w-full bg-[#F2F4F7] dark:bg-[#020617] text-gray-900 dark:text-white flex items-center justify-center overflow-hidden relative font-sans selection:bg-orange-500/30 transition-colors duration-500">
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div
            className="absolute bottom-[-20%] right-[-10%] w-[70%] h-[70%] bg-blue-400/20 dark:bg-blue-600/10 rounded-full blur-[120px] animate-float"
          ...
          {showRegister ? (
            <form onSubmit={handleRegisterSubmit} className="space-y-6">
              ...
              <Button
                ...
                isLoading={isLoading}
                className={cn(
                  "w-full h-14 text-lg font-bold mt-4 border-0 shadow-lg transition-transform active:scale-[0.98]",
                  selectedRole === "Admin"
          ...
          >
            ...
            <div className="relative z-10 h-full flex flex-col justify-between">
              ...
              <div>
                ...
                <p className="text-gray-500 dark:text-orange-200/60 font-medium">
                  Claims adjudication, fraud detection, and analytics.
                </p>
              </div>
          ...
          >
            ...
            <div className="relative z-10 h-full flex flex-col justify-between">
              ...
              <div>
                ...
                <p className="text-gray-500 dark:text-blue-200/60 font-medium">
                  Manage policies, file claims, and track status.
                </p>
              </div>
              <div className="w-full h-1 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
...
export default function App() {
  ...
  const [targetClaimId, setTargetClaimId] = useState<string | undefined>(
    undefined,
  );
  ...
  const handleLogin = async (role: UserRole) => {
    ...
    if (role === "User") {
      try {
        setToast({
          msg: `Welcome back, ${CURRENT_USER.name}`,
          type: "success",
        });
      } catch (err) {
    ...
    if (role === "Admin") {
      ...
      setNotifications(["Welcome to Vantage Insurance!"]);
    }
  ...
  const ADMIN_NAV: NavItem[] = [
    ...
    },
    {
      id: "settings",
      label: "Settings",
      icon: <SettingsIcon size={22} strokeWidth={1.5} />,
    },
  ];
      ...
      {userRole === "Admin" && (
        <div className="w-full bg-[#F97316] text-white py-2 px-8 flex items-center justify-center gap-3 shrink-0 z-[60] shadow-md border-b border-orange-600/50">
          ...
          <span className="text-[10px] sm:text-xs font-black uppercase tracking-[0.2em]">
            ADMIN PORTAL: You are viewing with elevated privileges
          </span>
        </div>
      )}
      ...
      >
        ...
        <div className="absolute inset-0 pointer-events-none opacity-100 dark:opacity-30 z-0 overflow-hidden transition-opacity duration-500">
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-[0.03] dark:opacity-[0.04]">
            <VantageLogo
              size={800}
              variant={userRole === "User" ? "blue" : "orange"}
              className="text-gray-900 dark:text-white animate-spin-slow transition-colors duration-500"
            />
          </div>
        </div>
            ...
            <AnimatePresence>
              {isSidebarOpen && (
                ...
                >
                  ...
                  <span className="font-semibold text-xl tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-[var(--primary)] to-[var(--primary-hover)] whitespace-nowrap">
                    Vantage
                  </span>
                </motion.div>
              )}
            ...
            >
              ...
              <AnimatePresence>
                {isSidebarOpen && (
                  ...
                  >
                    <p className="text-sm font-semibold truncate text-gray-900 dark:text-white transition-colors duration-300">
                      {CURRENT_USER.name}
                    </p>
                    <p className="text-xs text-gray-500 truncate transition-colors duration-300">
                      {CURRENT_USER.role}
                    </p>
                  </motion.div>
                )}
              ...
              <div className="relative">
                ...
                >
                  ...
                  {notifications.length > 0 && (
                    <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-[#F43F5E] rounded-full ring-2 ring-white dark:ring-[#0F172A] animate-pulse"></span>
                  )}
                </button>
                <AnimatePresence>
                  {showNotifications && (
                    ...
                    >
                      <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider px-4 py-3 bg-gray-50/50 dark:bg-white/5 mb-1 transition-colors duration-300">
                        Notifications
                      </h4>
                      {notifications.length === 0 ? (
                        ...
                        <div className="max-h-[300px] overflow-y-auto">
                          {notifications.map((n, i) => (
                            <div
                              key={i}
                              className="p-4 hover:bg-black/5 dark:hover:bg-white/5 rounded-2xl text-sm border-b border-gray-100 dark:border-white/5 last:border-0 transition-colors duration-300 mx-1"
                            >
                              {n}
                            </div>
                          ))}
