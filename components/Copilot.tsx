...
import {
    Send, Sparkles, X, Car, Heart, Activity, Home,
    ChevronDown, ChevronUp, CheckCircle2, AlertCircle, FileText,
    Download, MapPin, ArrowRight,
    UploadCloud, Layers, BookOpen
} from 'lucide-react';
...
const renderInlineMarkdown = (text: string): React.ReactNode => {
    ...
    const patterns = [
        { regex: /\*\*(.+?)\*\*/g, render: (match: string) => <strong key={keyIdx++} className="font-semibold text-slate-900 dark:text-white">{match}</strong> },
        { regex: /\*(.+?)\*/g, render: (match: string) => <em key={keyIdx++} className="italic">{match}</em> },
        { regex: /\(Source:\s*([^)]+)\)/g, render: (match: string) => <span key={keyIdx++} className="text-xs text-emerald-600 dark:text-emerald-400 font-medium block mt-1">(Source: {match})</span> },
    ];
    ...
    while ((match = boldRegex.exec(text)) !== null) {
        if (match.index > lastIndex) {
            parts.push(processItalicsAndSources(text.slice(lastIndex, match.index), keyIdx));
            keyIdx++;
        }
        parts.push(<strong key={`bold-${keyIdx++}`} className="font-semibold text-slate-900 dark:text-white">{match[1]}</strong>);
 ...
 */
const processItalicsAndSources = (text: string, baseKey: number): React.ReactNode => {
    if (!text) return null;
    ...
    while ((match = sourceRegex.exec(text)) !== null) {
        if (match.index > lastIndex) {
            parts.push(<span key={`text-${keyIdx++}`}>{text.slice(lastIndex, match.index)}</span>);
        }
        parts.push(
            <span key={`source-${keyIdx++}`} className="inline-flex items-center gap-1 text-[10px] bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded-full font-medium ml-1">
                <BookOpen size={10} />
                {match[1]}
            </span>
        );
...
const SUGGESTIONS: Record<Category, string[]> = {
    Vehicle: ["What does my policy cover?", "Start Accident Claim", "What's my deductible?", "Am I covered for theft?"],
    Health: ["What's covered under my plan?", "Check Deductible", "Submit Invoice", "Is surgery covered?"],
    Life: ["Who are my beneficiaries?", "Coverage Limit", "What's the payout process?", "Update Address"],
    Home: ["What damage is covered?", "Report Damage", "Is flooding covered?", "What's my premium?"],
};
...
export const Copilot = () => {
    ...
    const loadContext = useCallback(async () => {
        ...
        try {
            ...
            console.error('Copilot: failed to load context', err);
        }
    }, [contextLoaded]);
    ...
    const handleSend = async (text: string = input) => {
        ...
        const userMsg: Message = { id: Date.now().toString(), sender: 'user', text: text || 'Action selected', type: 'text' };
        ...
        try {
            ...
            if (status === 429) {
                ...
                errorText = "The AI service is currently unavailable. Please check that the API key is configured.";
            } else if (status === 401) {
                errorText = "Your session has expired. Please sign out and sign back in.";
    ...
    const SmartSummary = ({ data, category }: { data: any; category?: Category }) => {
        ...
        return (
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-white/5 space-y-3">
                <div className="flex justify-between items-center">
                    ...
                    <Badge variant="info" className="text-[9px]">{category}</Badge>
                </div>
                <p className="text-xs text-slate-500">Coverage Limit: <span className="font-bold text-slate-900 dark:text-white">{data.limit}</span></p>

                <button onClick={() => setIsExpanded(!isExpanded)} className="flex items-center gap-1 text-[10px] font-black uppercase text-orange-500 hover:text-orange-600 transition-colors">
                    {isExpanded ? <><ChevronUp size={12} /> Hide Benefits</> : <><ChevronDown size={12} /> View Benefits</>}
                </button>
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                            <ul className="space-y-1.5 mt-2 border-t pt-2 border-slate-50 dark:border-white/5">
                                {data.benefits.map((b: string, i: number) => (
                                    <li key={i} className="text-[11px] flex items-start gap-2 text-slate-600 dark:text-slate-400">
                                        <CheckCircle2 size={12} className="text-emerald-500 mt-0.5 shrink-0" /> {b}
                                    </li>
                                ))}
                            </ul>
                            <div className="flex gap-2 mt-4">
                                <Button variant="secondary" size="sm" className="flex-1 text-[10px] h-8">Compare Policies</Button>
                                <Button variant="secondary" size="sm" className="flex-1 text-[10px] h-8"><Download size={10} className="mr-1" /> PDF</Button>
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
    ...
    const ClaimWizard = ({ data, category }: { data: any; category?: Category }) => {
        ...
        return (
            <div className="bg-slate-50 dark:bg-white/5 rounded-2xl p-5 border border-slate-200 dark:border-white/10 space-y-4">
                <div className="flex justify-between items-center">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Claim Wizard • Step {step}/3</span>
                    <div className="flex gap-1">
                        {[1, 2, 3].map(s => <div key={s} className={cn("w-1.5 h-1.5 rounded-full transition-colors", s <= step ? "bg-orange-500" : "bg-slate-200")} />)}
                    </div>
                </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center gap-2 p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-white/5">
                                ...
                                <input type="text" placeholder="Location..." className="bg-transparent text-xs w-full outline-none" />
                                <CheckCircle2 size={14} className="text-emerald-500" />
                            </div>
                            <div className="flex items-center gap-2 p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-white/5">
                                <FileText size={14} className="text-slate-400" />
                                <input type="text" placeholder="Incident type..." className="bg-transparent text-xs w-full outline-none" />
                                <AlertCircle size={14} className="text-rose-500" />
                            </div>
                        </div>
                        <Button onClick={nextStep} className="w-full h-10 text-xs">Next <ArrowRight size={14} /></Button>
                    </div>
                )}
                ...
                {step === 2 && (
                    <div className="space-y-4">
                        ...
                        <div className="p-6 border-2 border-dashed border-slate-200 dark:border-white/10 rounded-2xl flex flex-col items-center justify-center gap-2 hover:border-orange-500/50 transition-colors cursor-pointer bg-white dark:bg-white/5">
                            ...
                            <span className="text-[10px] font-bold text-slate-400 uppercase">Upload Photos / Report</span>
                        </div>
                        ...
                        {ocrProgress > 0 && ocrProgress < 100 && (
                            <div className="space-y-1.5">
                                ...
                                <div className="h-1 bg-slate-200 dark:bg-white/10 rounded-full overflow-hidden">
                                    <motion.div initial={{ width: 0 }} animate={{ width: `${ocrProgress}%` }} className="h-full bg-orange-500" />
                                </div>
                            </div>
                        )}

                        <div className="space-y-1.5">
                            <div className="flex items-center gap-2 text-[11px] text-slate-500"><CheckCircle2 size={12} className="text-emerald-500" /> Photo of Damage</div>
                            <div className="flex items-center gap-2 text-[11px] text-slate-500"><div className="w-3 h-3 rounded-full border border-slate-300" /> Police Report</div>
                        </div>
                        <Button onClick={nextStep} disabled={ocrProgress < 100} className="w-full h-10 text-xs">Validate Documents</Button>
                    </div>
                )}
                ...
                {step === 3 && (
                    <div className="space-y-4 animate-in fade-in zoom-in-95 duration-500">
                        <div className="text-center pb-2">
                            ...
                            <p className="text-xs text-slate-500">Based on initial evidence scan</p>
                        </div>

                        <div className="p-4 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5">
                            <div className="flex justify-between items-end mb-1">
                                <span className="text-[10px] font-bold uppercase text-slate-400">Estimated Range</span>
                                <span className="text-lg font-black text-emerald-600">$1,200 - $1,500</span>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-4 text-[10px]">
                                <div className="p-2 bg-slate-50 dark:bg-white/5 rounded-lg">
                                    <span className="block font-bold text-slate-400 uppercase">Payable</span>
                                    <span className="text-emerald-600 font-bold">$1,350</span>
                                </div>
                                <div className="p-2 bg-slate-50 dark:bg-white/5 rounded-lg">
                                    <span className="block font-bold text-slate-400 uppercase">Deductible</span>
                                    <span className="text-rose-600 font-bold">$250</span>
                                </div>
                            </div>
                        </div>
                        <Button className="w-full h-12 bg-orange-500 text-white rounded-2xl border-0 shadow-lg shadow-orange-500/20">Finalize Submission</Button>
                    </div>
                )}
            </div>
        );
    ...
    return (
        <>
            <button
                ...
                className={`fixed bottom-6 right-6 p-4 rounded-full shadow-2xl z-50 transition-all duration-300 ${isOpen ? 'scale-0 opacity-0' : 'scale-100 opacity-100'
                        ...
                        <div className="p-5 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5">
                            <div className="flex justify-between items-center mb-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-black text-sm text-slate-800 dark:text-white uppercase tracking-wider">Vantage Copilot</h3>
                                ...
                                <button onClick={() => setIsOpen(false)} className="p-2 hover:bg-white dark:hover:bg-slate-800 rounded-full transition-colors border border-transparent hover:border-slate-100 shadow-sm">
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-5 custom-scrollbar bg-white dark:bg-[#0F172A]/50">
                            {messages.map((msg) => (
                                <div key={msg.id} className={cn("flex flex-col", msg.sender === 'user' ? 'items-end' : 'items-start')}>
                                    <div className={cn("max-w-[90%] space-y-1 group", msg.sender === 'user' ? 'items-end' : 'items-start')}>
                                        <div className={cn(
                                            ...
                                            {msg.type === 'summary' ? <SmartSummary data={msg.data} category={msg.category} /> :
                                                msg.type === 'wizard' ? <ClaimWizard data={msg.data} category={msg.category} /> :
                                        ...
                                        {msg.sender === 'ai' && msg.ragUsed && msg.sources && msg.sources.length > 0 && (
                                            <div className="flex items-center gap-1.5 px-2 py-1">
                                                ...
                                                <span className="text-[9px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">
                                                    Based on {msg.sources.length} document section{msg.sources.length !== 1 ? 's' : ''}
                                                </span>
                                            </div>
                                        )}
