 ...
 */
export const fetchClaims = async (filters?: { status?: string, min_risk_score?: number, skip?: number, limit?: number }): Promise<any[]> => {
...
export const updateClaimStatus = async (
    ...
    const response = await apiClient.patch<any>(`/claims/${id}/status`, { status });
    return transformClaim(response.data);
};
 ...
 */
export const runAiScan = async (claimId: string): Promise<RiskAnalysisResponse> => {
...
export const chatWithCopilot = async (
    ...
    const response = await apiClient.post<CopilotResponse>('/ai/copilot/chat', payload);
    return response.data;
};
 ...
 */
export const uploadDocument = async (claimId: string, file: File, documentType: string): Promise<any> => {
    const formData = new FormData();
    formData.append('file', file);

    const response = await apiClient.post(`/claims/${claimId}/documents?document_type=${encodeURIComponent(documentType)}`, formData, {
        headers: {
