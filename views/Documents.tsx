...
export const Documents = () => {
  ...
  useEffect(() => {
    const loadDocs = async () => {
      try {
        setIsLoading(true);
        setLoadError(null);
        const { fetchDocuments, fetchClaims } =
        ...
        claims.forEach((claim: any) => {
          ...
          (claim.documents || []).forEach((doc: any) => {
            ...
            const extended: ExtendedDocument = {
              ...
              userEmail:
                (existing ?? doc).user_email ?? (doc as any).user_email,
            };
  ...
  const handleSmartExtract = async (docId: string) => {
    ...
    if (doc) {
      ...
      setDocuments((prev) =>
        prev.map((d) =>
          d.id === docId ? { ...d, extractedEntities: entities } : d,
        ),
      );
    }
  ...
  const filteredDocs = documents.filter((doc) => {
    const matchesCategory =
      ...
      (selectedCategory === "Other"
        ? !doc.claimType
        : doc.claimType === selectedCategory);
    ...
    const matchesSearch =
      ...
      (doc.extractedEntities &&
        Object.values(doc.extractedEntities).some((v: any) =>
          (v as string).toLowerCase().includes(lowerQuery),
        ));
      ...
      <div className="relative z-10 flex flex-col lg:flex-row gap-8 flex-1 min-h-0">
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar pb-10 space-y-8">
          ...
          {Object.entries(groupedDocs).map(
            ([claimant, typeGroups]: [
              string,
              Record<string, ExtendedDocument[]>,
            ]) => (
              <div
                key={claimant}
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-6 sticky top-0 z-20 py-3 bg-white/10 dark:bg-black/10 backdrop-blur-lg border border-white/20 dark:border-white/5 shadow-sm transition-all rounded-2xl px-4">
                  ...
                  <Badge
                    variant="default"
                    className="bg-white/50 dark:bg-white/10 backdrop-blur-md border-white/20"
                  >
                    {Object.values(typeGroups).reduce(
                  ...
                  {Object.entries(typeGroups).map(([type, groupDocs]) => (
                    <div key={type} className="relative">
                      ...
                      <div className="flex items-center gap-2 mb-3 pl-2 border-l-2 border-orange-200 dark:border-orange-800 ml-1">
                        ...
                        <span className="text-xs text-gray-400 font-medium bg-gray-100 dark:bg-white/5 px-1.5 py-0.5 rounded-full">
                          {groupDocs.length}
                        </span>
                      </div>
                            ...
                            {groupDocs.map((doc) => (
                              ...
                              >
                                <Card className="h-full flex flex-col p-5 hover:shadow-lg transition-all group border border-gray-100 dark:border-white/5 bg-white/40 dark:bg-[#1C1C1E]/40 backdrop-blur-md">
                                  ...
                                  <h3
                                    className="font-semibold text-gray-900 dark:text-white truncate mb-1"
                                    title={doc.name}
                                  >
                                    {doc.name
                                      .toLowerCase()
                                  ...
                                  {doc.extractedEntities ? (
                                    <div className="mt-auto bg-[#F97316]/5 border border-[#F97316]/10 rounded-xl p-3 text-xs space-y-1">
                                      ...
                                      {Object.entries(
                                        ...
                                        <div
                                          key={k}
                                          className="flex justify-between border-b border-[#F97316]/5 pb-1 last:border-0"
                                        >
                                          ...
                                          <span
                                            className={cn(
                                              ...
                                              searchQuery &&
                                                ...
                                                ? "text-[#EA4335] bg-yellow-100 dark:bg-yellow-900/30 px-1 rounded"
                            ...
                            >
                              <div className="col-span-3 flex items-center gap-3 overflow-hidden">
                                ...
                                <div className="min-w-0">
                                  ...
                                  {searchQuery &&
                                    doc.extractedEntities &&
                                    Object.values(doc.extractedEntities).some(
                                      (v: any) =>
                                        (v as string)
                                          .toLowerCase()
                                          .includes(searchQuery.toLowerCase()),
                              ...
                              <div
                                className="col-span-3 text-sm text-gray-600 dark:text-gray-400 truncate pr-2"
                                title={doc.userEmail}
                              >
                                {doc.userEmail || "N/A"}
                              </div>
