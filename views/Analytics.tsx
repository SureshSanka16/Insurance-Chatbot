...
// Default fallback data if no claims available
...
const MetricCard = ({
  ...
  <Card className="h-[160px] p-6 flex flex-col justify-between shadow-lg border border-gray-200 dark:border-white/10 bg-white/70 dark:bg-[#1C1C1E]/40 backdrop-blur-2xl transition-all duration-300 hover:scale-[1.02]">
    <div className="flex justify-between items-start">
      ...
      <div
        className={cn(
          "text-[10px] font-black flex items-center gap-1 uppercase tracking-wider",
          trendClass,
        )}
      >
        {subtext}
    ...
    <div>
      <span className="font-black text-[10px] uppercase tracking-[0.2em] text-gray-400 block mb-1">
        {title}
      </span>
...
export const Analytics = () => {
  ...
  const [forecastData, setForecastData] = useState(DEFAULT_FORECAST_DATA);
  const [isLoadingData, setIsLoadingData] = useState(true);

  // Fetch claims data and calculate forecasts
  useEffect(() => {
    const fetchAndCalculateForecast = async () => {
      try {
        ...
        if (data && data.length > 0) {
          // Calculate real forecast data from claims
          const calculatedForecast = calculateForecastFromClaims(data);
          console.log("Calculated forecast data:", calculatedForecast);
          setForecastData(calculatedForecast);
        } else {
          console.log("No claims data, using default forecast");
  ...
  // Calculate monthly statistics and generate AI forecast
  const calculateForecastFromClaims = (claimsData: any[]) => {
    console.log("Calculating forecast from", claimsData.length, "claims");
    ...
    // Get current month (February = 1)
    const currentMonth = new Date().getMonth(); // 1 for February
    console.log(
      "Current month index:",
      currentMonth,
    ...
    // Calculate monthly claim counts for the past 12 months
    ...
    console.log("Monthly claim counts:", monthlyData);

    // Build forecast data: past 6 months as actual, future 3 months as forecast
    const result = [];

    for (let i = -6; i < 3; i++) {
      const monthIndex = (currentMonth + i + 12) % 12;
      const monthName = monthNames[monthIndex];
      const actualValue = monthlyData[monthName];

      if (i < 0) {
        // Past months - show actual data
        result.push({
          month: monthName,
          actual: actualValue > 0 ? actualValue : null,
          forecast: actualValue > 0 ? Math.round(actualValue * 1.05) : null, // Show trend line
        });
      } else {
        // Future months - show AI forecast (based on average growth)
        const avgPastVolume =
          Object.values(monthlyData).reduce((a, b) => a + b, 0) / 12;
        const growthFactor = 1 + i * 0.08; // 8% growth per month
        result.push({
          month: monthName,
          actual: null,
          forecast:
            avgPastVolume > 0
              ? Math.round(avgPastVolume * growthFactor)
              : Math.round(100 * growthFactor),
        });
      }
    }
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const report = await generateStrategicReport({
        claimsCount: actualClaimsCount > 0 ? actualClaimsCount : 1248,
        fraudValue: actualClaimsCount > 0 ? fraudValue : "$4.2M",
        riskDist:
          actualClaimsCount > 0
            ? actualRiskDist
            : [
  ...
  return (
    <div className="relative p-8 h-full flex flex-col gap-8 animate-in fade-in duration-700 max-w-full overflow-y-auto custom-scrollbar bg-transparent">
      ...
      <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 shrink-0 pl-1">
        <div className="space-y-1">
          ...
          <p className="text-lg text-slate-500 font-medium">
            Enterprise-grade forecasting and operational throughput monitoring.
          </p>
        </div>
        <div className="flex gap-4">
          <Button
            ...
            className="h-12 px-6 border-gray-200 dark:border-white/10 rounded-xl gap-2 shadow-sm bg-white dark:bg-white/5"
          >
            <Download size={16} />
          ...
          <Button
            ...
            className="h-12 px-6 shadow-xl bg-orange-600 hover:bg-orange-500 text-white border-0 rounded-xl gap-2 transition-transform active:scale-95"
          >
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full shrink-0">
        <MetricCard
          title="Total Claims"
          value={isLoadingData ? "..." : claims.length.toLocaleString()}
          subtext={
            <span>
              <Activity size={12} className="inline" /> Live Data
            </span>
          }
          icon={Activity}
        ...
        <MetricCard
          ...
          value={
            isLoadingData
              ? "..."
              : claims.filter((c) => c.riskScore >= 50).length.toString()
          }
          subtext={
            <span>
              <AlertTriangle size={12} className="inline" />{" "}
              {claims.length > 0
                ? `${((claims.filter((c) => c.riskScore >= 50).length / claims.length) * 100).toFixed(1)}%`
                : "0%"}
            </span>
          }
          icon={AlertTriangle}
        ...
        <MetricCard
          ...
          value={
            isLoadingData
              ? "..."
              : claims.filter((c) => c.status === "Approved").length.toString()
          }
          subtext={
            claims.length > 0
              ? `${((claims.filter((c) => c.status === "Approved").length / claims.length) * 100).toFixed(1)}% Rate`
              : "0% Rate"
          }
          icon={CheckCircle}
        ...
        <MetricCard
          ...
          value={
            isLoadingData
              ? "..."
              : `$${claims.length > 0 ? (claims.filter((c) => c.status === "Rejected").reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0) / 1000000).toFixed(1) : "0.0"}M`
          }
          subtext={
            <span>
          ...
          <div className="mb-8 flex justify-between items-start">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 uppercase tracking-[0.2em] font-black mt-0.5">
                  Predictive AI Projections • Fiscal Q4
                </p>
              </div>
            </div>
            <Badge
              variant="info"
              className="px-4 py-1.5 rounded-full font-black text-[9px] tracking-widest bg-blue-500/10 text-blue-600 border-blue-500/20"
            >
              LIVE DATA
            </Badge>
          </div>

          {console.log("Rendering chart with forecastData:", forecastData)}

          <div className="w-full h-[400px]">
            {isLoadingData ? (
              <div className="flex items-center justify-center h-full">
                <div className="flex flex-col items-center gap-4">
                  <div className="w-16 h-16 rounded-full border-4 border-orange-100 dark:border-orange-900/30 border-t-orange-500 animate-spin" />
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Area
                    ...
                    strokeWidth={3}
                    strokeDasharray="6 6"
                    fill="url(#colorForecastAnalytics)"
                    name="AI Baseline"
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-600 mt-2">
                    Data items: {forecastData?.length || 0}
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="flex flex-col gap-6 h-full min-h-[550px]">
          {isGeneratingReport ? (
            <div className="flex flex-col items-center justify-center flex-1 space-y-6 py-20">
              ...
              <div className="text-center">
                <h4 className="text-2xl font-black text-slate-900 dark:text-white mb-2 tracking-tighter">
                  Analyzing Market Signals
                </h4>
                <p className="text-sm text-slate-500 font-bold uppercase tracking-widest">
                  Compiling probabilistic data models...
                </p>
              </div>
            ...
            >
              <div className="flex justify-between items-start border-b border-gray-100 dark:border-white/5 pb-8">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-mono font-black uppercase tracking-widest mt-2">
                      REFERENCE: V-PRDX-8890
                    </p>
                  </div>
                </div>
                <div className="flex gap-3">
                  <Button
                    variant="secondary"
                    size="sm"
                    className="h-10 px-4 rounded-xl border-gray-200 dark:border-white/10 bg-white dark:bg-white/5"
                  >
                    <Share2 size={14} />
                  </Button>
                  <Button
                    size="sm"
                    onClick={handleExport}
                    className="h-10 px-5 rounded-xl bg-orange-600 hover:bg-orange-500 text-white border-0 shadow-lg font-black text-[10px] tracking-widest uppercase"
                  >
                    Export PDF
                  </Button>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-6 rounded-3xl bg-emerald-500/5 border border-emerald-500/10">
                  ...
                  <p className="text-xs font-bold text-slate-800 dark:text-slate-200">
                    AI confidence levels remain high at 92.4% for predicted Q4
                    volume spikes.
                  </p>
                </div>
                <div className="p-6 rounded-3xl bg-blue-500/5 border border-blue-500/10">
                  ...
                  <p className="text-xs font-bold text-slate-800 dark:text-slate-200">
                    Expansion of auto-approval logic for low-risk vehicle claims
              ...
              <Button
                variant="secondary"
                className="w-full h-14 rounded-2xl border-gray-200 dark:border-white/10 font-black tracking-widest uppercase text-xs bg-white dark:bg-white/5"
                onClick={() => setIsReportOpen(false)}
              >
                Dismiss Insight Intelligence
              </Button>
            </motion.div>
          )}
        </div>
