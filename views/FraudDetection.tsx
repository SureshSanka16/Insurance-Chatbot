...
import { ShieldAlert, Zap, TrendingUp, AlertTriangle, CheckCircle, Activity, FileText, Share2, Search, Filter } from 'lucide-react';
...
export const FraudDetection = () => {
    ...
    useEffect(() => {
        const loadClaims = async () => {
            try {
                ...
                console.error('Failed to load claims for fraud detection:', err);
            }
        };
        loadClaims();
    }, []);
    ...
    const rejectedTotal = claims.filter(c => c.status === 'Rejected').reduce((acc, c) => acc + (Number(c.amount) || 0), 0);
    const avgRiskScore = claims.length > 0 ? Math.round(claims.reduce((acc, c) => acc + (Number(c.riskScore) || 0), 0) / claims.length) : 0;
    const highRiskCount = claims.filter(c => (Number(c.riskScore) || 0) >= 50).length;
    ...
    return (
        <div className="p-8 h-full flex flex-col gap-8 overflow-y-auto custom-scrollbar bg-transparent">
             ...
             <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 shrink-0">
                <div>
                   <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-2 tracking-tight">Fraud Monitor</h1>
                   <p className="text-gray-500 font-medium">Real-time threat detection and anomaly analytics.</p>
                </div>
                <Button onClick={runAnalysis} className="bg-gradient-to-r from-rose-600 to-orange-600 hover:from-rose-500 hover:to-orange-500 border-0 shadow-lg shadow-rose-500/20 px-8 h-12 rounded-2xl">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 shrink-0">
                 ...
                 <StatCard 
                    ...
                    value={rejectedTotal.toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 })} 
                    change={claims.filter(c => c.status === 'Rejected').length > 0 ? `${claims.filter(c => c.status === 'Rejected').length} rejected` : undefined} 
                    icon={CheckCircle} 
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-[400px]">
                ...
                <Card className="flex flex-col p-6 bg-white/80 dark:bg-[#1C1C1E]/80 backdrop-blur-2xl border border-gray-100 dark:border-white/10 shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white">Threat Mitigation Trends</h3>
                            <p className="text-xs text-gray-500">Detected vs Prevented incidents over 6 months</p>
                        </div>
                        <Badge variant="success">98.5% Efficacy</Badge>
                    </div>
                    <div className="flex-1 w-full min-h-[300px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={FRAUD_TRENDS} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                <defs>
                                ...
                                />
                                <Area type="monotone" dataKey="detected" stroke="#F43F5E" strokeWidth={3} fillOpacity={1} fill="url(#colorDetected)" name="Threats Detected" />
                                <Area type="monotone" dataKey="prevented" stroke="#10B981" strokeWidth={3} fillOpacity={1} fill="url(#colorPrevented)" name="Fraud Prevented" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </Card>
                ...
                <Card className="flex flex-col p-6 bg-white/80 dark:bg-[#1C1C1E]/80 backdrop-blur-2xl border border-gray-100 dark:border-white/10 shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white">Alert Categories</h3>
                            <p className="text-xs text-gray-500">Distribution of flagged anomalies</p>
                        </div>
                    </div>
                    <div className="flex-1 w-full min-h-[300px]">
                        <ResponsiveContainer width="100%" height="100%">
                            {/* Adjusted margins: increased top/bottom to compress chart area and reduce gap, added bottom margin for edge spacing */}
                            <BarChart data={ALERT_TYPES} layout="vertical" margin={{ left: 0, right: 30, top: 20, bottom: 40 }}>
                                <XAxis type="number" hide />
                                <YAxis dataKey="name" type="category" width={100} tick={{fill: '#6B7280', fontSize: 11, fontWeight: 600}} axisLine={false} tickLine={false} />
                                <Tooltip 
                    ...
                    <Card className="p-0 overflow-hidden bg-white/80 dark:bg-[#1C1C1E]/80 backdrop-blur-2xl border border-gray-100 dark:border-white/10 shadow-sm">
                        ...
                        <div className="divide-y divide-gray-100 dark:divide-white/5">
                            {MOCK_ALERTS.map(alert => (
                                <div key={alert.id} className="p-4 flex items-start gap-4 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors cursor-pointer group">
                                    <div className={cn(
                                        "w-10 h-10 rounded-xl flex items-center justify-center shrink-0",
                                        alert.severity === 'High' ? "bg-rose-100 text-rose-600 dark:bg-rose-900/20 dark:text-rose-400" :
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start mb-1">
                                            <h4 className="text-sm font-bold text-gray-900 dark:text-white truncate">{alert.type}</h4>
                                        ...
                                        <div className="flex gap-2 mt-2">
                                            {alert.relatedClaims.map(id => (
                                                <span key={id} className="text-[10px] px-1.5 py-0.5 bg-gray-100 dark:bg-white/10 rounded text-gray-500 font-mono">{id}</span>
                                            ))}
                                        </div>
                                    </div>
                                    <div className="self-center opacity-0 group-hover:opacity-100 transition-opacity">
                                        <Button variant="secondary" size="sm" className="h-8 w-8 p-0 rounded-full"><Share2 size={14}/></Button>
                                    </div>
                                </div>
                            ))}
                        </div>
                            ...
                            <div className="flex-1 bg-white/10 rounded-2xl p-4 backdrop-blur-md border border-white/10 text-sm leading-relaxed text-indigo-100 overflow-y-auto custom-scrollbar max-h-[250px] mb-4 whitespace-pre-wrap">
                                {analyzing ? (
                                    <div className="flex flex-col items-center justify-center h-full gap-3 opacity-70">
                                        ...
                                        <span className="text-xs font-bold uppercase tracking-widest">Processing Data...</span>
                                    </div>
                                ) : (
                                    aiAnalysis || "Run a scan to generate a detailed threat assessment report based on current claim vectors."
                                )}
                            </div>

                            <Button onClick={runAnalysis} className="w-full bg-white text-indigo-900 hover:bg-indigo-50 border-0 font-bold shadow-lg">
