...
const OFFERED_POLICIES = [
  {
    ...
    premium: 2000,
    features: ["Term Life", "Critical Illness", "Total Disability"],
  },
  {
    category: "Health",
    title: "Health Shield H-500",
    coverage_amount: 500000,
    premium: 500,
    features: ["Cashless", "OPD Cover", "No Claim Bonus"],
  },
  {
    category: "Vehicle",
    title: "Drive Secure V-15",
    coverage_amount: 1500000,
    premium: 800,
    features: ["Zero Dep", "Roadside Assist", "Engine Protect"],
  },
  {
    category: "Property",
    title: "Home Protect P-50",
    coverage_amount: 5000000,
    premium: 1200,
    features: ["Fire & Burglary", "Content Cover", "Natural Calamity"],
  },
];
...
const DocSection = ({ title, emoji, children }: any) => (
  <div className="mb-6">
    <h3 className="text-[10px] font-black uppercase text-gray-400 tracking-widest border-b border-gray-100 pb-1 mb-3">
      {emoji} {title}
    </h3>
    {children}
  </div>
...
const ClaimTracker = ({ claim }: { claim: any }) => {
  ...
  const steps = [
    ...
    {
      label: claim.status === Status.Rejected ? "Rejected" : "Approved",
      status:
        claim.status === Status.Rejected ? Status.Rejected : Status.Approved,
    },
  ];
  ...
  const displayStatus =
    claim.status === Status.Analyzing ? Status.InReview : claim.status;

  if (displayStatus === Status.InReview || displayStatus === Status.Flagged)
    currentStep = 1;
    ...
    <div className="w-full">
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {claim.type} • Filed on {claim.date}
          </p>
        </div>
        <div className="text-right">
      ...
      <div className="relative mb-6 px-4">
        ...
        <motion.div
          ...
          className={cn(
            "absolute top-1/2 left-0 h-1.5 -translate-y-1/2 rounded-full transition-colors duration-500",
            progressColor,
          )}
        />
        <div className="relative flex justify-between z-10 w-full">
          {steps.map((step, index) => {
            ...
            return (
              <div
                key={index}
                className="flex flex-col items-center -ml-4 first:ml-0 last:mr-0"
              >
                <motion.div
                  animate={{
      ...
      >
        ...
        <div>
          ...
          >
            {displayStatus === Status.New &&
              "Your claim has been received and queued for assignment."}
            {displayStatus === Status.InReview &&
              "Our intelligent systems are analyzing the evidence."}
            {displayStatus === Status.Flagged &&
              "We need a few more details to process your claim."}
            {displayStatus === Status.Approved &&
              "Great news! Your claim has been approved."}
            {displayStatus === Status.Rejected &&
              "Unfortunately, your claim does not meet criteria."}
          </p>
        </div>
      </div>
    </div>
...
const PolicyCard: React.FC<{
  ...
  activeClaim?: Claim;
  onViewPolicy: () => void;
  onNewClaim: (e: React.MouseEvent) => void;
}> = ({ policy, activeClaim, onViewPolicy, onNewClaim }) => {
  ...
  return (
    <motion.div whileHover={{ y: -5 }} className="h-full">
      <Card
        className={cn(
          "h-full flex flex-col justify-between p-0 overflow-hidden",
          theme.bg,
          theme.border,
        )}
      >
        <div className="p-6 flex-1">
          <div className="flex justify-between items-start mb-6">
            <div
              className={cn(
                "p-4 rounded-2xl bg-white dark:bg-white/10 shadow-md border border-white/20",
                theme.iconColor,
              )}
            >
          ...
          <div className="mb-4">
            ...
            <p className="text-[10px] font-mono text-gray-400 mt-1.5 font-bold tracking-widest uppercase">
              {policy.policyNumber}
            </p>
          </div>
        ...
        <div className="p-4 pt-0 grid grid-cols-1 sm:grid-cols-2 gap-2">
          ...
          <Button
            ...
            className={cn(
              "rounded-xl h-10 border-0 shadow-lg font-bold transition-all",
              theme.button,
              !!activeClaim && "opacity-70 grayscale",
            )}
          >
            {activeClaim ? (
              <>
                <FileBadge size={14} /> {activeClaim.status}
              </>
            ) : (
              <>
...
const MANDATORY_DOCS: Record<PolicyCategory, string[]> = {
  ...
  Property: [
    "Damage Photographs",
    "Repair Estimate / Contractor Quotation",
    "Ownership Proof / Property Deed",
    "Incident Report (Fire / Police / Society)",
  ],
  Life: [
    "Death Certificate",
    "Claimant (Nominee) ID Proof",
    "Policy Document",
  ...
  Travel: [
    ...
    "Ticket Details",
    "Boarding Pass",
    "Incident Report (Police/Airline)",
  ],
};
    ...
    >
      ...
      <div className="flex items-center gap-4 overflow-hidden">
        <div
          className={cn(
            ...
            uploadedFile
              ...
              : "bg-gray-100 dark:bg-white/10 text-gray-400 group-hover:bg-blue-100 group-hover:text-blue-600",
          )}
        >
        ...
        <div className="flex flex-col overflow-hidden">
          ...
          {uploadedFile ? (
            <span className="text-[10px] font-mono text-emerald-600 opacity-80 truncate">
              {uploadedFile.name}
            </span>
          ) : (
      ...
      {uploadedFile && (
        <div className="flex items-center gap-2">
          <Badge
            variant="success"
            className="bg-emerald-100 border-0 text-emerald-700"
          >
            Ready
          </Badge>
          <button
...
export const UserDashboard = () => {
  ...
  const [wizardStep, setWizardStep] = useState<
    "info" | "details" | "documents" | "breakdown" | "preview"
  >("info");
  ...
  const [formData, setFormData] = useState<any>({
    ...
    propertyAddress: "",
    propertyIncidentType: "Fire",
    locationOfDamage: "",
    fireDeptInvolved: false,
    reportNumber: "",
  });
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setPolicies(
          policiesData.filter(
            (p: Policy) => p.title !== "Global Health Shield",
          ),
        );
  ...
  const handleBuyPolicy = async (plan: any) => {
    ...
    try {
      ...
      const [policiesData] = await Promise.all([fetchPolicies()]);
      setPolicies(
        policiesData.filter((p: Policy) => p.title !== "Global Health Shield"),
      );
      setToast({
        msg: `Successfully registered for ${plan.title}!`,
        type: "success",
      });
    } catch (error: any) {
  ...
  const addItem = () =>
    setFormData({
      ...formData,
      items: [...formData.items, { item: "", cost: 0 }],
    });
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      if (
        ...
        formData.items.some((i: any) => i.item)
      ) {
      ...
      if (cat === "Vehicle") {
        ...
        claimPayload.health_info = {
          ...
          admissionDate:
            formData.admissionDate || formData.incidentDate || undefined,
        ...
        claimPayload.life_info = {
          ...
          dateOfDeath:
            formData.dateOfDeath || formData.incidentDate || undefined,
        ...
        claimPayload.property_info = {
          ...
          locationOfDamage:
            formData.locationOfDamage || formData.location || undefined,
      ...
      if (newClaim && newClaim.id) {
        const uploadPromises = Object.entries(formData.uploadedDocs).map(
          async ([docType, file]) => {
            if (file) {
              try {
        ...
        try {
          ...
          // Failed to finalize claim - don't fail the whole process
        }
      }
      ...
      setWizardStep("info");
      setToast({
        msg: "Claim submitted successfully with documents!",
        type: "success",
      });
      ...
      try {
        ...
        // Failed to refresh claims list, but claim was created successfully
        ...
        >
          <div>
            ...
            <p className="text-gray-500 font-medium">
              Policy management for {CURRENT_USER.name}
            </p>
          </div>
        </motion.div>
          ...
          {userClaims.length > 0 && (
            <div className="pt-2">
              <div className="flex items-center justify-between mb-6">
                ...
                <Badge variant="info">
                  {userClaims.length}{" "}
                  {userClaims.length === 1 ? "Claim" : "Claims"}
                </Badge>
              </div>

              {/* Group claims by category */}
              {(() => {
                const claimsByPolicy = userClaims.reduce(
                  (acc, claim) => {
                ...
                return Object.entries(claimsByPolicy).map(
                  ([groupName, claims]) => {
                            ...
                            >
                              <div className="grid grid-cols-1 gap-4">
                                {claims.map((claim) => (
                                  <Card
                                    key={claim.id}
                                    className="relative overflow-hidden p-5 border-gray-200 dark:border-gray-700 shadow-lg bg-white/95 dark:bg-white/5 backdrop-blur-3xl hover:shadow-xl transition-shadow"
                                  >
            ...
            {showMarketplace ? (
              <div className="space-y-6">
                <div className="p-6 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-500/20 text-center mb-8 relative">
                  ...
                  <p className="text-gray-600 dark:text-gray-300">
                    {policies.length === 0
                      ? "You don't have any active policies yet. Choose a plan below to get started instantly."
                      : "Explore our premium insurance plans tailored for your needs."}
                  </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {OFFERED_POLICIES.map((plan, idx) => (
                    <Card
                      key={idx}
                      className="flex flex-col h-full border-dashed border-2 hover:border-solid hover:border-blue-500 transition-all group"
                    >
                      <div className="p-6 flex-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {policies.map((policy) => {
                  ...
                  const activeClaim = userClaims.find(
                    (c) =>
                      c.policyNumber === policy.policyNumber &&
                ...
                <Card
                  ...
                  className="flex flex-col items-center justify-center p-6 border-dashed border-2 cursor-pointer hover:border-blue-500 hover:bg-blue-50/50 transition-all min-h-[300px] group"
                >
                  ...
                  <p className="text-sm text-gray-500 mt-2 text-center">
                    Explore comprehensive plans for Health, Life, Vehicle, and
                    Property.
                  </p>
                </Card>
              </div>
        ...
        {selectedPolicy && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-white dark:bg-[#1C1C1E] rounded-[32px] overflow-hidden shadow-2xl"
            >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedPolicy.features.map((f) => (
                      <Badge key={f} variant="info" className="lowercase">
                        {f}
                      </Badge>
                    ))}
                  </div>
      ...
      <Modal
        ...
        title={
          wizardStep === "preview"
            ...
            : `New ${claimingPolicy?.category} Claim`
        }
        size={wizardStep === "preview" ? "lg" : "md"}
      >
        <div className="flex flex-col gap-6">
          <div className="flex items-center justify-center gap-3 mb-4">
            {["info", "details", "documents", "breakdown", "preview"].map(
              (step) => (
                <div
                  key={step}
                  className={cn(
          ...
          {wizardStep === "info" && (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-white/5">
                <Button
                  variant="secondary"
                  onClick={() => setClaimingPolicy(null)}
                >
                  Cancel
                </Button>
                <Button
                  ...
                  className="px-8"
                >
                  {claimingPolicy?.category} Details <ChevronRight size={16} />
                </Button>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">
                  {claimingPolicy?.category} Details{" "}
                  <span className="text-gray-300 font-medium normal-case">
                    (All fields optional — fill what you can)
                  </span>
                </p>
              </div>
                ...
                <div className="space-y-4 bg-blue-50/30 dark:bg-blue-900/10 p-4 rounded-2xl border border-blue-100 dark:border-blue-500/10">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <Select
                        value={formData.vehicleIncidentType}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            vehicleIncidentType: e.target.value,
                          })
                        }
                      >
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        placeholder="If filed"
                        value={formData.policeReportNo}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            policeReportNo: e.target.value,
                          })
                        }
                      />
                    </div>
              ...
              {claimingPolicy?.category === "Health" && (
                <div className="space-y-4 bg-red-50/30 dark:bg-red-900/10 p-4 rounded-2xl border border-red-100 dark:border-red-500/10">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={formData.surgeryPerformed}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          surgeryPerformed: e.target.checked,
                        })
                      }
                      className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    ...
                    <div className="space-y-1">
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            nomineeRelationship: e.target.value,
                          })
                        }
                      >
                        <option value="Spouse">Spouse</option>
                ...
                <div className="space-y-4 bg-green-50/30 dark:bg-green-900/10 p-4 rounded-2xl border border-green-100 dark:border-green-500/10">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <FormLabel icon={AlertTriangle}>Incident Type</FormLabel>
                      <Select
                        value={formData.propertyIncidentType}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            propertyIncidentType: e.target.value,
                          })
                        }
                      >
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        value={formData.locationOfDamage}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            locationOfDamage: e.target.value,
                          })
                        }
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <Input
                        placeholder="Fire/Police report no."
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={formData.fireDeptInvolved}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          fireDeptInvolved: e.target.checked,
                        })
                      }
                      className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    <span className="text-xs text-gray-600 dark:text-gray-400">
                      Fire department was involved
                    </span>
                  </label>
                </div>
              )}
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              <div className="bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-500/20 p-4 rounded-2xl flex items-center gap-3">
                ...
                <p className="text-xs text-blue-700 dark:text-blue-300 font-medium">
                  All mandatory documents are required to proceed.
                </p>
              </div>
              <div className="space-y-3">
                {claimingPolicy &&
                  MANDATORY_DOCS[claimingPolicy.category].map((docName) => (
                    <UploadSlot
                      key={docName}
                      label={docName}
                      uploadedFile={formData.uploadedDocs[docName]}
                      onFileSelect={(fileName) =>
                        setDocumentFile(docName, fileName)
                      }
                    />
                  ))}
              </div>
          ...
          {wizardStep === "breakdown" && (
            <div className="space-y-6 animate-in fade-in duration-300">
              <div className="p-4 bg-gray-50 dark:bg-black/20 rounded-2xl border border-gray-100 dark:border-white/5">
                <h4 className="text-[10px] font-black uppercase text-gray-400 mb-4">
                  Loss Items
                </h4>
                <div className="space-y-3">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={addItem}
                    className="w-full border-dashed rounded-xl mt-2"
                  >
              ...
              <div className="flex justify-between gap-3 pt-4 border-t border-gray-100 dark:border-white/5">
                ...
                <Button
                  ...
                  className="px-8 bg-blue-600 text-white border-0"
                >
                  Preview <Sparkles size={16} />
                </Button>
              </div>
            </div>
          )}
          ...
          {wizardStep === "preview" && (
            <div className="space-y-6 animate-in zoom-in-95 duration-500">
              <div className="bg-white p-8 rounded-[32px] border-4 border-gray-50 shadow-inner max-h-[500px] overflow-y-auto custom-scrollbar">
                <DocHeader
                  title={`${claimingPolicy?.category} Claim Form`}
                  claimNo="PENDING"
                  date={new Date().toLocaleDateString()}
                  status="DRAFT"
                  icon={claimingPolicy?.category === "Vehicle" ? Car : Heart}
                  emoji="📁"
                />
                <DocSection title="Policyholder Details" emoji="👤">
                  <DocGrid
                    items={[
                      { label: "Full Name", value: CURRENT_USER.name },
                      {
                        label: "Policy Number",
                        value: claimingPolicy?.policyNumber,
                      },
                    ]}
                  />
                </DocSection>
                <DocSection title="Incident Summary" emoji="📍">
                  <DocGrid
                    items={[
                ...
                <DocSection title="Description" emoji="📝">
                  <p className="text-[11px] italic text-gray-600 bg-gray-50 p-3 rounded-lg">
                    {formData.description || "Not Provided"}
                  </p>
                </DocSection>
                <DocSection title="Uploaded Files" emoji="📎">
                  <div className="grid grid-cols-2 gap-2">
                    {Object.keys(formData.uploadedDocs).map((d) => (
                      <div
                        key={d}
                        className="p-2 bg-gray-50 rounded-lg text-[10px] flex items-center gap-1"
                      >
                        <FileCheck size={12} className="text-emerald-500" /> {d}
                      </div>
                    ))}
                  </div>
                </DocSection>
                <div className="mt-8 border-t border-dashed pt-4 flex justify-between items-end">
                  <div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-white/5">
                ...
                <Button
                  ...
                  className="bg-emerald-600 text-white border-0 px-10"
                >
                  Confirm & Submit <FileBadge size={16} />
                </Button>
              </div>
            </div>
          )}
      ...
      <AnimatePresence>
        {selectedPolicy &&
          (() => {
            ...
            const policyClaims = userClaims.filter(
                        ...
                        <DocGrid
                          items={[
                            ...
                            },
                            {
                              label: "Contact",
                              value:
                                selectedClaim.phoneNumber || "Not Provided",
                            },
                      ...
                      <DocSection title="Claim Details" emoji="📋">
                        <DocGrid
                          items={[
                            {
                              label: "Claim Amount",
                              value: `$${selectedClaim.amount.toLocaleString()}`,
                            },
                            {
