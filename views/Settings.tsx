...
const SettingRow = ({
  ...
  <div
    className="flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors cursor-pointer first:rounded-t-2xl last:rounded-b-2xl bg-white dark:bg-white/5 border-b border-gray-100 dark:border-white/5 last:border-0"
    onClick={onClick}
  >
      ...
      {type === "toggle" && (
        <div
          className={cn(
            ...
            active ? "text-[#34A853]" : "text-gray-300",
          )}
        >
...
const ActivityHistory = ({ onBack }: { onBack: () => void }) => {
  ...
  const userClaims = MOCK_CLAIMS.filter(
    (c) => c.claimant === CURRENT_USER.name,
  );

  const isUser = CURRENT_USER.role === "User";
  const accentColor = isUser ? "text-[#2563EB]" : "text-[#F97316]";
  const accentBg = isUser
  ...
  return (
    <div className="space-y-6">
      <button
        onClick={onBack}
        className={cn(
          "flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-opacity-80 transition-colors mb-4",
          isUser ? "hover:text-blue-600" : "hover:text-orange-500",
        )}
      >
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Recent activity and claim decisions.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {userClaims.length === 0 ? (
          ...
          userClaims.map((claim) => (
            <Card
              key={claim.id}
              className="p-5 flex items-center justify-between hover:shadow-lg transition-all border border-gray-100 dark:border-white/5"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5 line-clamp-1 max-w-[200px] md:max-w-md">
                    {claim.description}
                  </p>
...
export const Settings = ({ onLogout }: SettingsProps) => {
  ...
  const [fraudMode, setFraudMode] = useState<
          ...
          <div>
            ...
            <div className="flex flex-col gap-0.5 mt-1">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.15em] text-gray-400">
                Verified{" "}
                {userRole === "Admin" ? "Administrator" : "Policyholder"}
              </p>
            </div>
            <button
              onClick={() => setIsProfileOpen(true)}
              className={cn(
                "text-xs font-bold mt-4 px-4 py-1.5 rounded-full bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 transition-all",
                accentColor,
              )}
            >
              Edit Display Info
            ...
            {CURRENT_USER.role === "Admin" && (
              <div className="bg-white dark:bg-white/5 border-b border-gray-100 dark:border-white/5 p-4 flex flex-col gap-4">
                ...
                <p className="text-[10px] text-gray-400 italic">
                  {fraudMode === "Aggressive" &&
                    "Higher sensitivity: Catches more potential fraud but may increase false positives."}
                  {fraudMode === "Balanced" &&
                    "Standard sensitivity: Recommended for optimal operations."}
                  {fraudMode === "Conservative" &&
                    "Lower sensitivity: Focuses on clearly identified risk patterns only."}
                </p>
              </div>
            )}
            <SettingRow
      ...
      >
        <div className="space-y-4">
          ...
          {CURRENT_USER.role === "Admin" && (
            <div>
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1 block">
                Role (Admin Overwrite)
              </label>
              <select
                value={userRole || ""}
                onChange={(e) => setUserRole(e.target.value as any)}
                className={cn(
                  "w-full h-12 p-3 bg-gray-50 dark:bg-white/5 rounded-2xl border border-gray-200 dark:border-white/10 focus:ring-2 outline-none",
                  focusRing,
                )}
              >
      ...
      >
        <div className="space-y-6">
          <p className="text-xs text-gray-500 font-medium">
            Update your verified contact and address information for policy
            accuracy.
          </p>
          <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-100 dark:border-white/5">
            ...
            <Button onClick={handleSavePersonalDetails} className="flex-1">
              Update Details
            </Button>
          </div>
        </div>
          ...
          {!is2FAEnabled && !show2FAQR ? (
            <div className="flex flex-col items-center text-center py-4">
              ...
              <p className="text-sm text-gray-500 max-w-xs mb-8">
                ...
                your Vantage account by requiring a code from your phone.
              </p>
              <Button
                className="w-full h-14 rounded-2xl bg-orange-600 text-white border-0 shadow-lg shadow-orange-500/20"
            ...
            <div className="space-y-6 animate-in zoom-in-95 duration-300">
                ...
                <div className="grid grid-cols-6 gap-2">
                  {[1, 2, 3, 4, 5, 6].map((i) => (
                    <input
                      ...
                      type="text"
                      maxLength={1}
                      className="w-full h-12 text-center text-xl font-bold bg-white dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-orange-500 outline-none touch-action-none"
                      placeholder="0"
                    />
                  ))}
                </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button
                  ...
                  className="flex-1 bg-orange-600 text-white border-0"
                >
                  Verify & Activate
                </Button>
              </div>
            ...
            <div className="flex flex-col items-center text-center py-6">
              ...
              <p className="text-sm text-gray-500 max-w-xs mb-8">
                Your account is protected by an additional security layer. You
                will be prompted for a code on new sign-ins.
              </p>
              <Button
                variant="danger"
